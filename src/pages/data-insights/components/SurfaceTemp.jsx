...
const SurfaceTemp = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // FIXED: Correct path - remove 'src/' from the path
          const response = await fetch('/global_surface_temp_NOAA_grid_2005_2025.csv');
        ...
        Papa.parse(csvText, {
          header: true,
          complete: (results) => {
            console.log('CSV parsed, rows:', results.data.length); // Debug log
            ...
            const parsedData = results.data
              ...
              .map(row => ({
                ...
                month_str: new Date(row.date).toISOString().slice(0, 7)
              }))
              .filter(item => !isNaN(item.lat) && !isNaN(item.lon) && !isNaN(item.temp_anomaly_c));

            console.log('Parsed data points:', parsedData.length); // Debug log
  ...
  useEffect(() => {
    ...
    console.log('Initializing plot with', data.length, 'data points'); // Debug log
    ...
    Plotly.newPlot(plotRef.current, traces, layout, config).then(() => {
      console.log('Plot created, adding frames'); // Debug log
      Plotly.addFrames(plotRef.current, frames);
      if (months.length > 0) {
        setCurrentMonth(months[0]);
        setCurrentIndex(0);
        setTimeout(() => {
          console.log('Plot ready!'); // Debug log
          setPlotReady(true);
        }, 500);
      }
    }).catch(err => {
  ...
  const handlePlayPause = () => {
    if (!plotRef.current || !plotRef.current.data || plotRef.current.data.length === 0) {
      console.warn('Plot not ready, retrying...'); // Debug log
      setTimeout(() => {
        if (plotRef.current && plotRef.current.data && plotRef.current.data.length > 0) {
          handlePlayPause();
        }
    ...
    if (isPlaying) {
      ...
      const animateFrame = () => {
        if (!animationActive || !plotRef.current) return;
  ...
  const handleSliderChange = (index) => {
    if (!plotRef.current || !plotRef.current.data) return;
          ...
          <div className="max-w-7xl mx-auto bg-gray-900/40 backdrop-blur-xl rounded-2xl p-4 border border-cyan-500/30 shadow-2xl shadow-cyan-500/10">
            <div className="flex items-center justify-between">
              <button
                ...
                disabled={!plotReady}
                className={`px-6 py-3 rounded-xl font-semibold text-base transition-all duration-300 transform hover:scale-105 border ${!plotReady
              ...
              >
                <span className="flex items-center space-x-2">
                  <span className="text-sm">{!plotReady ? 'Loading...' : isPlaying ? 'Pause' : 'Play'}</span>
                </span>
              </button>
            ...
            <div className="flex justify-between mb-3 px-1 pt-4">
              {years.map((year) => (
                <button
                  ...
                  className={`year-button text-cyan-400 text-xs font-semibold bg-gray-800/40 px-3 py-2 rounded-md border transition-all duration-200 ${currentYear === year
